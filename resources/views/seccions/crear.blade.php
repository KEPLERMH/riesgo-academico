...
@section('content')
    @if (Auth::user()->tipo == 1)
        ...
        <div class="modal-body" id="modal-create">
            <h3 class="text-center" style="margin-bottom: 55px">REGISTRO DE NUEVA CARRERA</h3>
            <form action="{{ route('seccions.store') }}" method="POST">
                ...
                <div class="form-row col-12">
                    @error('seccion')
                        <small>*{{ $message }}</small>
                    @enderror
                    <input type="text" placeholder="Example: SISTEMAS-INDUSTRIAL" name="seccion" id="seccion"
                        value="{{ old('seccion') }}">
                </div>
